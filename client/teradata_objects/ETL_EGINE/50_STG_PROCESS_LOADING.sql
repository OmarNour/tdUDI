REPLACE  PROCEDURE /*VER.04*/ GDEV1_ETL.STG_PROCESS_LOADING
    (
    ...
    BEGIN
		...
		DECLARE		V_LAYER_NAME				VARCHAR(10) DEFAULT 'STG';
        ...
        DECLARE 	V_INSERTED_ROWS_COUNT,V_UPDATED_ROWS_COUNT,V_DELETED_ROWS_COUNT,
        			V_ROWS_IN_1batch, V_ROWS_IN_DELTA, V_ROWS_COUNT, v_UniqueFlag_count, v_batch_id_as_PI	FLOAT DEFAULT 0;
		...
		DECLARE		V_IS_TARANSACTIOANL, v_large_object_count, v_recreate_stg_table_required	INTEGER default 0;
        ...
        BEGIN
			...
			THEN				
				...
				if v_batch_id_as_PI = 0
				then
					set v_recreate_stg_table_required = 1;					
				end if;
			...
			if v_recreate_stg_table_required > 0
			then
				set v_recreate_stg_table = 'drop table '||V_TGT_DB||'.'||i_TABLE_NAME||'_TEMPpi;';
				CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_recreate_stg_table,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,0/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
				
				IF I_ALL_BATCHES = 1
				then
					set v_recreate_stg_table = 'create multiset table '||V_TGT_DB||'.'||i_TABLE_NAME||'_TEMPpi as '||V_TGT_DB||'.'||i_TABLE_NAME||' with data primary index('||V_KEY_COLs||');';
				else
					set v_recreate_stg_table = 'create multiset table '||V_TGT_DB||'.'||i_TABLE_NAME||'_TEMPpi as '||V_TGT_DB||'.'||i_TABLE_NAME||' with no data primary index('||V_KEY_COLs||');';
				end if;
				
				SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
				CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_recreate_stg_table,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
				...
				IF I_ALL_BATCHES = 0
				then
					set v_recreate_stg_table = 'insert into '||V_TGT_DB||'.'||i_TABLE_NAME||'_TEMPpi select * from '||V_TGT_DB||'.'||i_TABLE_NAME||' '||V_QUALIFY ;
					SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
					CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_recreate_stg_table,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
					IF V_DBC_RETURN_CODE <> 0 THEN
						SET V_RETURN_CODE = V_DBC_RETURN_CODE;
				...
				set v_recreate_stg_table = 'rename table '||V_TGT_DB||'.'||i_TABLE_NAME||' to '||V_TGT_DB||'.'||i_TABLE_NAME||'_dropTemp;';
				SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
				CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_recreate_stg_table,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
				IF V_DBC_RETURN_CODE <> 0 THEN
					SET V_RETURN_CODE = V_DBC_RETURN_CODE;
				...
				set v_recreate_stg_table = 'rename table '||V_TGT_DB||'.'||i_TABLE_NAME||'_TEMPpi to '||V_TGT_DB||'.'||i_TABLE_NAME||';';
				SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
				CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_recreate_stg_table,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
				IF V_DBC_RETURN_CODE <> 0 THEN
					SET V_RETURN_CODE = V_DBC_RETURN_CODE;
				...
				set v_recreate_stg_table = 'drop table '||V_TGT_DB||'.'||i_TABLE_NAME||'_dropTemp;';
				SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
				CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_recreate_stg_table,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
				IF V_DBC_RETURN_CODE <> 0 THEN
					SET V_RETURN_CODE = V_DBC_RETURN_CODE;
            ...
            then
            	...
            	SET V_UPDATE_ALL_TO_D = 'update '||V_TGT_DB||'.'||i_TABLE_NAME||' TGT
										set modification_type = ''D''
			...
			CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_drop_1batch,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,0/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
			CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_drop_delta,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,0/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);

			
			SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
			CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_create_1batch,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
			IF V_DBC_RETURN_CODE <> 0 THEN
				SET V_RETURN_CODE = V_DBC_RETURN_CODE;
				SET V_RETURN_MSG = V_DBC_RETURN_MSG;
				LEAVE MainBlock;
			...
			then
				SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
				CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(V_UPDATE_ALL_TO_D,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
				IF V_DBC_RETURN_CODE <> 0 THEN
					SET V_RETURN_CODE = V_DBC_RETURN_CODE;
					SET V_RETURN_MSG = V_DBC_RETURN_MSG;
					LEAVE MainBlock;
			...
			SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
			CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_create_delta,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
			CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_drop_1batch,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,0/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
			IF V_DBC_RETURN_CODE <> 0 THEN
				SET V_RETURN_CODE = V_DBC_RETURN_CODE;
				SET V_RETURN_MSG = V_DBC_RETURN_MSG;
			...
			SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
			CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG('BT',V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
			IF V_DBC_RETURN_CODE <> 0 THEN
				SET V_RETURN_CODE = V_DBC_RETURN_CODE;
				SET V_RETURN_MSG = V_DBC_RETURN_MSG;
				LEAVE MainBlock;
			...
			then
				SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
				CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_delete_from_tgt,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
				IF V_DBC_RETURN_CODE <> 0 THEN
					SET V_RETURN_CODE = V_DBC_RETURN_CODE;
					SET V_RETURN_MSG = V_DBC_RETURN_MSG;
					LEAVE MainBlock;
			...
			then
				SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
				CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_update_tgt,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
				IF V_DBC_RETURN_CODE <> 0 THEN
					SET V_RETURN_CODE = V_DBC_RETURN_CODE;
					SET V_RETURN_MSG = V_DBC_RETURN_MSG;
					LEAVE MainBlock;
			...
			then
				SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
				CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG(v_insert_into_tgt,V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
				IF V_DBC_RETURN_CODE <> 0 THEN
					SET V_RETURN_CODE = V_DBC_RETURN_CODE;
					SET V_RETURN_MSG = V_DBC_RETURN_MSG;
					LEAVE MainBlock;
			...
			SET V_SQL_SCRIPT_ID = V_SQL_SCRIPT_ID + 1;
			CALL GDEV1_ETL.DBC_SYSEXECSQL_WITH_LOG('ET',V_SQL_SCRIPT_ID,v_run_id,V_SOURCE_NAME,V_PROCESS_NAME,I_LOAD_ID,1/*1 log or 0 don't*/,V_DBC_RETURN_CODE,V_DBC_RETURN_MSG,V_DBC_ROWS_COUNT,v_dbc_run_id);
			IF V_DBC_RETURN_CODE <> 0 THEN
				SET V_RETURN_CODE = V_DBC_RETURN_CODE;
				SET V_RETURN_MSG = V_DBC_RETURN_MSG;
				LEAVE MainBlock;
